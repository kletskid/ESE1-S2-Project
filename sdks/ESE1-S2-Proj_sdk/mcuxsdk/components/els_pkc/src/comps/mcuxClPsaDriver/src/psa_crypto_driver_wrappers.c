...
#define PSA_CRYPTO_MBED_TLS_DRIVER_ID (1u)

#if defined(PSA_CRYPTO_DRIVER_TEST)
#define PSA_CRYPTO_TRANSPARENT_TEST_DRIVER_ID (2u)
#define PSA_CRYPTO_OPAQUE_TEST_DRIVER_ID (3u)
#endif /* PSA_CRYPTO_DRIVER_TEST */
    ...
    size_t *hash_length)
{
    ...
    status = mcuxClPsaDriver_psa_driver_wrapper_hash_compute(alg, input, input_length, hash, hash_size, hash_length);
    if(PSA_ERROR_NOT_SUPPORTED != status)
    {
        return status;
    }
    ...
    if(PSA_ERROR_CORRUPTION_DETECTED  == status)
    {
...
#ifdef PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT
#ifdef PSA_CRYPTO_DRIVER_TEST
...
#ifdef MBEDTLS_PSA_BUILTIN_AEAD
        case PSA_CRYPTO_MBED_TLS_DRIVER_ID:
   ...
   size_t tag_length)
{
    switch(operation->id)
    {
        ...
        case PSA_CRYPTO_MBED_TLS_DRIVER_ID:
            {
                psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
                uint8_t check_tag[PSA_AEAD_TAG_MAX_SIZE] = {0};
                size_t check_tag_length = 0u;
...
#if defined(PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT)
#if defined(PSA_CRYPTO_DRIVER_TEST)
//MCUX TFM    *tag_len = operation->ctx.transparent_test_driver_ctx.tag_length;
//MCUX TFM    return ( PSA_SUCCESS );
//MCUX TFM#endif
//MCUX TFM#endif
//MCUX TFM    *tag_len = operation->ctx.mbedtls_ctx.tag_length;
//MCUX TFM    return ( PSA_SUCCESS );
        case PSA_CRYPTO_TRANSPARENT_TEST_DRIVER_ID:
