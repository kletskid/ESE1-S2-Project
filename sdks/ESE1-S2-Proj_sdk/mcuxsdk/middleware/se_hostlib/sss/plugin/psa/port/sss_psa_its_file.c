...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C) && AX_EMBEDDED
    psa_status_t psa_status = PSA_ERROR_STORAGE_FAILURE;
    uint8_t data[4096]      = {0};
    size_t dataLen          = sizeof(data);
    if (PSA_KEY_ID_NEEDS_ITS_SE(uid)) {
        psa_status = psa_alt_read_se_its_file(uid, data, &dataLen);
    }
    else if (PSA_KEY_ID_NEEDS_ITS_FLASH(uid)) {
    ...
    psa_storage_uid_t uid, uint32_t data_length, const void *p_data, psa_storage_create_flags_t create_flags)
{
    ...
    if (PSA_KEY_ID_NEEDS_ITS_SE(uid)) {
        status = psa_alt_store_se_its_file(uid, p_store_data, dataLen);
    }
    else if (PSA_KEY_ID_NEEDS_ITS_FLASH(uid)) {
...
psa_status_t psa_its_remove(psa_storage_uid_t uid)
{
    ...
    if (PSA_KEY_ID_NEEDS_ITS_SE(uid)) {
        psa_status = psa_alt_remove_se_its_file(uid);
    }
    else if (PSA_KEY_ID_NEEDS_ITS_FLASH(uid)) {
