...
#define SSS_ENABLE_RSA_ALT_VERIFY 0

/*
    Note: KeyId - 0x7D000021 is used to store the public key and do the RSA Verify.
    Assuming first time a rsa1024 key is set, only a rsa1024 key can be overwritten here for next RSA Verify.
    Any other key type if used, will result in error during set key.

    Solution -
    Enable `SSS_DELETE_SE05X_TMP_RSA_PUBLIC_KEY` to delete the key after every RSA Verify.
    This will ensure a new key is created next time and the implementation will work for all RSA key types.
    NOTE THAT, THIS WILL RESULT IN NVM FLASH WRITES.
*/
#define SSS_DELETE_SE05X_TMP_RSA_PUBLIC_KEY 0
#define SSS_SE05X_TMP_RSA_PUBLIC_KEY_ID 0x7D000021
...
static sss_key_store_t *g_rsa_keystore    = NULL;
ex_sss_boot_ctx_t g_rsa_se05x_session_ctx = {0};

void sss_mbedtls_set_keystore_rsa(sss_key_store_t *ssskeystore)
{
    g_rsa_keystore = ssskeystore;
}
...
int mbedtls_rsa_complete(mbedtls_rsa_context *ctx)
{
    ...
    /*
        Ignore the check for Reference key
        TBD - Use the magic number for check instead
    */
    if (ctx->Q.n == 1) {
        return 0;
    }
 ...
 * Do an RSA operation to sign the message digest
 */
int mbedtls_rsa_pkcs1_sign_o(mbedtls_rsa_context *ctx,
                           int (*f_rng)(void *, unsigned char *, size_t),
                           void *p_rng,
                           mbedtls_md_type_t md_alg,
                           unsigned int hashlen,
...
{
    uint8_t qpBuf[256]    = {0};
    size_t qpBuf_len            = sizeof(qpBuf);
    uint8_t magic_number[8] = {0xB6, 0xB5, 0xA6, 0xA5, 0xB6, 0xB5, 0xA6, 0xA5};
    int mbedtls_ret         = 0;

    LOG_D("mbedtls_rsa_pkcs1_sign (%s)", __FILE__);

    ENSURE_OR_RETURN_ON_ERROR(ctx != NULL, 1);
    ENSURE_OR_RETURN_ON_ERROR(hash != NULL, 1);
    ENSURE_OR_RETURN_ON_ERROR(sig != NULL, 1);
    (void)f_rng;
    (void)p_rng;

    qpBuf_len = mbedtls_mpi_size(&ctx->QP);
    mbedtls_ret = mbedtls_mpi_write_binary_le(&ctx->QP, qpBuf, qpBuf_len);
    ENSURE_OR_RETURN_ON_ERROR(mbedtls_ret == 0, mbedtls_ret);

    ENSURE_OR_RETURN_ON_ERROR(qpBuf_len >= 4, 1);

    if (!memcmp(qpBuf, magic_number, 4)) /* Check for first 4 bytes */
    {
        int ret                    = 1;
        uint8_t ref_key[4]         = {0};
        ...
        uint32_t key_id            = 0;
        size_t keyLen              = ctx->len;
        sss_asymmetric_t ctx_asymm = {0};
        sss_object_t keyPair       = {0};
        sss_algorithm_t algorithm  = kAlgorithm_None;

        LOG_I("Reference key found. Use Secure element for RSA.");

        ref_key_len = mbedtls_mpi_size(&ctx->Q);
        ENSURE_OR_GO_EXIT(ref_key_len <= sizeof(ref_key));
        mbedtls_ret = mbedtls_mpi_write_binary_le(&ctx->Q, ref_key, ref_key_len);
        ENSURE_OR_RETURN_ON_ERROR(mbedtls_ret == 0, mbedtls_ret);

        key_id = ref_key[3];
        key_id = key_id << (1 * 8);
        ENSURE_OR_GO_EXIT(key_id <= UINT32_MAX - ref_key[2]);
        key_id += ref_key[2];
        key_id = key_id << (1 * 8);
        ENSURE_OR_GO_EXIT(key_id <= UINT32_MAX - ref_key[1]);
        key_id += ref_key[1];
        key_id = key_id << (1 * 8);
        ENSURE_OR_GO_EXIT(key_id <= UINT32_MAX - ref_key[0]);
        key_id += ref_key[0];
        ...
        */
        if (!g_rsa_keystore) {
            ...
            LOG_W(
                "NOTE: Opening the se05x sesion now. \
                    Use the sss_mbedtls_set_keystore_rsa() api to pass the keystore if required.");

            status = ex_sss_boot_connectstring(0, NULL, &portName);
            ENSURE_OR_GO_EXIT(kStatus_SSS_Success == status);

            status = ex_sss_boot_open(&g_rsa_se05x_session_ctx, portName);
            ENSURE_OR_GO_EXIT(kStatus_SSS_Success == status);

            status = ex_sss_key_store_and_object_init(&g_rsa_se05x_session_ctx);
            ENSURE_OR_GO_EXIT(kStatus_SSS_Success == status);

            status = sss_key_object_init(&keyPair, &g_rsa_se05x_session_ctx.ks);
            ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
        }
        else {
            status = sss_key_object_init(&keyPair, g_rsa_keystore);
            ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
        }
        ...
        LOG_I("RSA Sign using SE05x (Using key id %02X)", keyPair.keyId);
        status = sss_asymmetric_sign_digest(&ctx_asymm, (uint8_t *)hash, hashlen, sig, &keyLen);
        ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    exit:
        ...
        if (g_rsa_keystore == NULL) {
            LOG_I("Close SE05x session");
            ex_sss_session_close(&g_rsa_se05x_session_ctx);
        }
        return ret;
    }
    else {
        LOG_I("Not a reference key. Rollback to software implementation of rsa sign");
        return mbedtls_rsa_pkcs1_sign_o(ctx, f_rng, p_rng, md_alg, hashlen, hash, sig);
    }
}
 ...
 */
int mbedtls_rsa_pkcs1_verify_o(mbedtls_rsa_context *ctx,
                             mbedtls_md_type_t md_alg,
                             unsigned int hashlen,
                             const unsigned char *hash,
                             const unsigned char *sig)
...
int mbedtls_rsa_pkcs1_verify(mbedtls_rsa_context *ctx,
                             ...
                             const unsigned char *sig)
{
#if SSS_ENABLE_RSA_ALT_VERIFY
    int ret                    = 1;
    sss_status_t status        = kStatus_SSS_Success;
    size_t keyLen              = ctx->len;
    uint32_t key_id            = SSS_SE05X_TMP_RSA_PUBLIC_KEY_ID;
    sss_asymmetric_t ctx_asymm = {0};
    sss_object_t keyPair       = {0};
    sss_algorithm_t algorithm  = kAlgorithm_None;
    uint8_t publickey[550]     = {0};
    ...
    case MBEDTLS_MD_SHA512:
        ...
        break;
    default:
        LOG_E("Invalid md algorithm");
        goto exit;
    }

    /* Create a asn1 format public key  */
    switch (keyLen) {
    case 64:
        memcpy(publickey, grsa512PubHeader, der_rsa_512_len);
        ENSURE_OR_GO_EXIT(sizeof(publickey) > der_rsa_512_len);
        publickeylen += der_rsa_512_len;
        keyBitLen = 64 * 8;
        break;
    case 128:
        memcpy(publickey, grsa1kPubHeader, der_rsa_1024_len);
        ENSURE_OR_GO_EXIT(sizeof(publickey) > publickeylen + der_rsa_1024_len);
        publickeylen += der_rsa_1024_len;
        keyBitLen = 128 * 8;
        break;
    case 144:
        memcpy(publickey, grsa1152PubHeader, der_rsa_1152_len);
        ENSURE_OR_GO_EXIT(sizeof(publickey) > publickeylen + der_rsa_1152_len);
        publickeylen += der_rsa_1152_len;
        keyBitLen = 144 * 8;
        break;
    case 256:
        memcpy(publickey, grsa2kPubHeader, der_rsa_2048_len);
        ENSURE_OR_GO_EXIT(sizeof(publickey) > publickeylen + der_rsa_2048_len);
        publickeylen += der_rsa_2048_len;
        keyBitLen = 256 * 8;
        break;
    case 384:
        memcpy(publickey, grsa3kPubHeader, der_rsa_3072_len);
        ENSURE_OR_GO_EXIT(sizeof(publickey) > publickeylen + der_rsa_3072_len);
        publickeylen += der_rsa_3072_len;
        keyBitLen = 384 * 8;
        break;
    case 512:
        memcpy(publickey, grsa4kPubHeader, der_rsa_4096_len);
        ENSURE_OR_GO_EXIT(sizeof(publickey) > publickeylen + der_rsa_4096_len);
        publickeylen += der_rsa_4096_len;
        keyBitLen = 512 * 8;
        break;
    default:
        LOG_E("Wrong RSA key length");
        goto exit;
    }

    /* appending length of public key*/
    if (keyLen < 0x80) { // rsa512
        publickey[publickeylen++] = (uint8_t)keyLen;
    }
    else if (keyLen < 0x100) { // rsa1024
        publickey[publickeylen++] = 0x81;
        publickey[publickeylen++] = 0x81;
        publickey[publickeylen++] = 0x00;
    }
    else { // other keys
        publickey[publickeylen++] = 0x82;
        memcpy(publickey + publickeylen, &keyLen, 2);
        publickeylen++;
        publickey[publickeylen - 1] = publickey[publickeylen] ^ publickey[publickeylen - 1];
        publickey[publickeylen]     = publickey[publickeylen - 1] ^ publickey[publickeylen];
        publickey[publickeylen - 1] = publickey[publickeylen] ^ publickey[publickeylen - 1];
        publickeylen++;
    }

    ENSURE_OR_GO_EXIT(SIZE_MAX - publickeylen >= mbedtls_mpi_size(&ctx->N));
    ENSURE_OR_GO_EXIT(sizeof(publickey) > publickeylen + mbedtls_mpi_size(&ctx->N));
    mbedtls_ret = mbedtls_mpi_write_binary(&ctx->N, publickey + publickeylen, mbedtls_mpi_size(&ctx->N));
    ENSURE_OR_RETURN_ON_ERROR(mbedtls_ret == 0, mbedtls_ret);
    publickeylen += mbedtls_mpi_size(&ctx->N);

    ENSURE_OR_GO_EXIT(sizeof(publickey) > publickeylen + 2);
    /* appending tag of public exponent*/
    publickey[publickeylen++] = 2;
    /* appending length of public exponent*/
    publickey[publickeylen++] = 3;

    /* appending value of public exponent*/
    ENSURE_OR_GO_EXIT(sizeof(publickey) + publickeylen > mbedtls_mpi_size(&ctx->E));
    mbedtls_ret = mbedtls_mpi_write_binary(&ctx->E, publickey + publickeylen, mbedtls_mpi_size(&ctx->E));
    ENSURE_OR_RETURN_ON_ERROR(mbedtls_ret == 0, mbedtls_ret);
    publickeylen += mbedtls_mpi_size(&ctx->E);
    ...
    */
    if (!g_rsa_keystore) {
        ...
        LOG_W(
            "NOTE: The ALT implementation will open and close the session. All transient objects will be lost.\n \
               To avoid the session open in ALT, Use the sss_mbedtls_set_keystore_rsa() api to pass the keystore.");
        status = ex_sss_boot_connectstring(0, NULL, &portName);
        ENSURE_OR_GO_EXIT(kStatus_SSS_Success == status);
    ...
    status = sss_key_object_allocate_handle(
        &keyPair, key_id, kSSS_KeyPart_Public, kSSS_CipherType_RSA_CRT, ctx->len, kKeyObject_Mode_Transient);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

    status = sss_key_store_set_key(keyPair.keyStore, &keyPair, publickey, publickeylen, keyBitLen, NULL, 0);
    if (status != kStatus_SSS_Success) {
        LOG_I(
            "The key id might have been used for storing different type of key \n. \
            Delete the key and try again OR use a different key id. (SSS_SE05X_TMP_RSA_PUBLIC_KEY_ID)");
        goto exit;
    }
    ...
    LOG_I("RSA Verify using SE05x (Using key id %02X)", keyPair.keyId);
    status = sss_asymmetric_verify_digest(&ctx_asymm, (uint8_t *)hash, hashlen, (uint8_t *)sig, keyLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

#if SSS_DELETE_SE05X_TMP_RSA_PUBLIC_KEY
    LOG_I("Delete key %x", SSS_SE05X_TMP_RSA_PUBLIC_KEY_ID);
...
exit:
    ...
    return ret;
#else
    return mbedtls_rsa_pkcs1_verify_o(ctx, md_alg, hashlen, hash, sig);
#endif
}
