                              ...
                              const mbedtls_mpi *d)
{
    ...
    if (mbedtls_ecp_get_type(grp) == MBEDTLS_ECP_TYPE_SHORT_WEIERSTRASS) {
        ...
        if (mbedtls_mpi_write_binary_le(d, ref_key, ref_key_len) != 0) {
            return MBEDTLS_ERR_ECP_INVALID_KEY;
        }

        if(!memcmp(&ref_key[2], magic_number, sizeof(magic_number))){
            /* If it is a reference key, igonore the other checks */
            return 0;
        }
        else if (mbedtls_mpi_cmp_int(d, 1) < 0 ||
