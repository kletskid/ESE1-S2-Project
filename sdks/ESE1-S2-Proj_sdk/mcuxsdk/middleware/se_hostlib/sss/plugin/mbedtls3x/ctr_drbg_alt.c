...
#define SSS_ENABLE_RND_ALT 0

static sss_key_store_t *g_rng_keystore    = NULL;
ex_sss_boot_ctx_t g_rng_se05x_session_ctx = {0};
                            ...
                            size_t output_len)
{
    ...
    if (!g_rng_keystore) {
        LOG_W(
            ...
            To avoid the session open in ALT, Use the sss_mbedtls_set_keystore_rng() api to pass the keystore.");

        status = ex_sss_boot_connectstring(0, NULL, &portName);
        ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

        status = ex_sss_boot_open(&g_rng_se05x_session_ctx, portName);
        ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

        status = ex_sss_key_store_and_object_init(&g_rng_se05x_session_ctx);
        ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

        status = sss_rng_context_init(&rng_ctx, &g_rng_se05x_session_ctx.session);
        ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    }
    else {
    ...
    LOG_I("Random number generation using SE05x");
...
exit:
    ...
    if (!g_rng_keystore) {
        ...
        ex_sss_session_close(&g_rng_se05x_session_ctx);
    }
