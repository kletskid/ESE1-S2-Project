 ...
 *  Copyright (C) 2022-2024, NXP, All Rights Reserved
...
#if defined(SMCOM_JRCP_V2) && SSS_HAVE_SE05X_VER_GTE_07_02
#include "smCom.h"
#include "smComJRCP.h"
#include "sm_types.h"
#endif
...
usage:
    ...
    if (strlen(opt.ca_path)) {
        ...
        ret = mbedtls_x509_crt_parse(&cacert, (const unsigned char *)rootca_file, rootca_file_len);
    } else
#endif
...
#else /* Bare-Metal OS */
    if (strlen(opt.crt_file)) {
        ret = mbedtls_x509_crt_parse(&clicert, (const unsigned char *)devcert_file, devcert_file_len);
    } else
#endif
#if defined(MBEDTLS_CERTS_C)
...
#else /* Bare-Metal OS */
    if (strlen(opt.key_file)) {
        ret = mbedtls_pk_parse_key(&pkey, (const unsigned char *)devkey_file, devkey_file_len, NULL, 0);
    } else
#endif
#if defined(MBEDTLS_CERTS_C)
    ...
    {
        /*
         * On K64, mbedtls_net_connect api will not take ip address.
         * So using Lwip apis to directly to connect.
         */
        ...
        ip4addr_aton(opt.server_addr, ip_2_ip4(&dstaddr));

        server_fd.fd = lwip_socket(AF_INET, SOCK_STREAM, 0);

		addr.sin_len = sizeof(addr);
		addr.sin_family = AF_INET;
		addr.sin_port = PP_HTONS(atoi(opt.server_port));
		inet_addr_from_ip4addr(&addr.sin_addr, ip_2_ip4((const ip_addr_t*)&dstaddr));

		ret = lwip_connect(server_fd.fd, (struct sockaddr*)&addr, sizeof(addr));
		if (ret < 0)
		{
			goto exit;
		}
